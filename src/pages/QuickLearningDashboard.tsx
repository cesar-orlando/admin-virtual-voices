...
const QuickLearningDashboard: React.FC = () => {
    ...
    if (unreadMessages.size > 0) {
      ...
      if (lastUnreadEntry) {
        ...
        const prospect = prospects.find(
          p => formatPhoneNumber(p.data?.telefono || '') === lastUnreadPhone
        )
  ...
  // Filtro memoizado de prospectos con filtro de IA
  const filteredProspects = React.useMemo(() => {
    ...
    if (iaFilter === 'withIA') {
      return list.filter(p => p.data.aiEnabled === true)
    }
    if (iaFilter === 'withoutIA') {
      return list.filter(p => !p.data.aiEnabled)
    }
    return list
  }, [prospects, searchTerm, iaFilter])
    ...
    if (selectedProspect) {
      try {
        ...
        const updatedProspect =
          prospects.find(p => p._id === selectedProspect._id) || selectedProspect
  ...
  // Ref para controlar el scroll de la lista virtualizada
  const listRef = useRef<any>(null)

  // Cuando cambia el filtro de IA, haz scroll arriba (opcional UX tipo WhatsApp)
  useEffect(() => {
    if (listRef.current) {
  ...
  return (
    ...
    >
            ...
            <Box sx={{ width: '100%', mt: 1, display: 'flex', justifyContent: 'center' }}>
              ...
              >
                ...
                <Button
                  ...
                  sx={{ fontWeight: 700, borderRadius: 3, textTransform: 'none', px: 3 }}
                  startIcon={<AIIcon sx={{ color: '#7B61FF' }} />}
                >
                  Con IA
                </Button>
                <Button
          ...
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, p: 0 }}>
            {!selectedProspect ? (
              <Box sx={{ textAlign: 'center', color: 'text.secondary', mt: 8 }}>
                <WhatsAppIcon sx={{ fontSize: 64, mb: 2 }} />
                    ...
                    >
                      ...
                      <Box sx={{ marginLeft: 1 }}>
                        <Typography variant="h6" fontWeight={700}>
                          {selectedProspect.data?.nombre
                            ? selectedProspect.data.nombre.trim()
                  ...
                  >
                    {chatHistory.length === 0 ? (
                      ...
                      [...chatHistory].map((msg, idx) => {
                        ...
                        if (
                          body.startsWith('🖼️ El usuario compartió una imagen:') &&
                          body.includes('https://')
                        ) {
                          ...
                          body.startsWith('🎥 El usuario compartió un video:') &&
                          body.includes('https://')
                        ) {
                          ...
                          body.startsWith('📍 El usuario compartió su ubicación:') &&
                          body.includes('https://')
                        ) {
                          const url = body
                            .replace('📍 El usuario compartió su ubicación:', '')
                            .trim()
                          content = (
                            <a
                              href={url}
                          ...
                          body.startsWith(
                            'Aquí tienes la información para realizar tu pago a Quick Learning'
                          )
                        ) {
                          content = (
                            <>
                      ...
                      if (typingIndicator) {
                        const typingText = (() => {
                          switch (typingIndicator.userType) {
                            ...
                            case 'bot':
                              return 'NatalIA está escribiendo...'
                            case 'asesor':
                              return 'Asesor está escribiendo...'
                            default:
                              return 'está escribiendo...'
                          }
                        })()
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setSendMessageDialog(false)}>Cancelar</Button>
          <Button
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setOpenImageModal(false)} color="primary" variant="contained">
            Cerrar
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
          ...
          {paymentMethods.map(method => (
            <Box
              ...
              sx={{
                ...
                border: '1px solid #E0E0E0',
                backgroundColor: '#fff',
                boxShadow: '0 2px 12px 0 rgba(123,97,255,0.08)',
              }}
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Avatar
                  sx={{
                    ...
                    bgcolor: '#7B61FF',
                    color: '#fff',
                    boxShadow: '0 2px 8px 0 rgba(123,97,255,0.12)',
                  }}
                >
                  <PaymentIcon sx={{ fontSize: 30 }} />
                </Avatar>
                <Box>
                  <Typography variant="subtitle1" fontWeight={700} color="#7B61FF">
                    {method.label}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {method.description}
              ...
              <Button
                ...
                onClick={async () => {
                  const phone = selectedProspect?.data?.telefono || selectedProspect?.phone
                  const nombre =
                    selectedProspect?.data?.nombre || selectedProspect?.data?.name || 'amigo/a'
                  console.log('DEBUG: Enviando plantilla de pago', {
                    phone,
        ...
        <DialogActions sx={{ justifyContent: 'center', pb: 2 }}>
          <Button
            ...
            sx={{ color: '#7B61FF', fontWeight: 700, fontSize: 16 }}
          >
            Cancelar
          </Button>
        </DialogActions>
      ...
      {/* Loader de scroll infinito solo al final y solo si hay más prospectos */}
      {isLoadingMoreProspects && hasMoreProspects && (
